...
- **Cover images** (per group): `groups.image_model` set (per-group ONLY — no env fallback, opt-in) + an active `first`-kind template → slide 1 = cover with generated image (`{{image}}` data-URI token), last slide = `last`-kind template (CTA). Image generated once per post (`posts/<id>/cover.png` in MinIO, reused on rerender); generation failure = fail-safe (renders without cover). Reels ignore kind (query filters `kind='body'`). Telegram command menu auto-registered at boot.
