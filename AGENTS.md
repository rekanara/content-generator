...
- **Rerender** (`/rerender [group]` bot, FE button, POST /posts/:id/rerender): same content re-rendered with the CURRENT template (no LLM). sent → restore status to `sent` + resend (rotation untouched); awaiting → awaiting + fresh buttons; rendered → gate on: awaiting + buttons, gate off: deliver (first send, rotation advances). Refuses failed/rejected/draft/queued and text format. **Regenerate** (POST /posts/:id/regenerate, awaiting/rejected/failed): awaiting is rejected first (rotation-safe), then a fresh plan-aware generate run — FE parity with the Telegram Regenerate button.
- **Promotions regen/rerender**: POST /promotions/:id/regenerate re-writes the slides from the same promo data (template switchable — same or different promo template, `setContentWithTemplate` pins it; image slots recompute, uploads persist per index). Refuses sent promos (storage must not lie about what shipped). POST /promotions/:id/rerender re-renders + resends a SENT promo with the current template row (edits apply) via the `promoSend` queue job.
- **Star** (`posts.starred`, FE star button on sent posts, Telegram star button on approved messages): manual quality signal — starred topics are shown to the AI planner as "resonated" and mark style-sample candidates.
- **Telegram UX**: approval buttons terminal-stamp the origin message via `editMessageReplyMarkup` (no zombie buttons); approval offers Approve / Regenerate (reject+re-gen, rotation-safe) / Reject; post-approve leaves a Star button. `/status` shows awaiting count, idea backlog, cost today. Daily digest (07:00 WIB, `usecases/digest.ts`): today+tomorrow slot specs (plan-merged), awaiting, yesterday recap+cost, idea count.
